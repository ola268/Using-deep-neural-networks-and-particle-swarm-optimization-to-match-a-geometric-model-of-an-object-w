...
// generates wf double between (0, 1)
    ...
    for (j=0; j<settings->size; j++) {
        ...
        for (i=0; i<settings->size; i++)
            ...
            if (comm[i*settings->size + j] && fit_b[i] < fit_b[b_n])
                // found wf better informer for j^th particle
                b_n = i;
...
// topology initialization :: this is wf static (i.e. fixed) topology
	       ...
	       pso_result_t *solution, pso_settings_t *settings)
{
    ...
    int i, d, step;
    double wb, b; // for matrix initialization
    ...
    for (i=0; i<settings->size; i++) {
        ...
        for (d=0; d<settings->dim; d++) {
            ...
            wb = settings->range_lo[d] + (settings->range_hi[d] - settings->range_lo[d]) * \
                RNG_UNIFORM();
    ...
    for (step=0; step<settings->steps; step++) {
        ...
        // do not bother with calling wf calc_w_const function
